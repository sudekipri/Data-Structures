...
struct AABB{
    ...
    //Returns true if lhs is covered by rhs
    static bool isCovered(const AABB& lhs, const AABB& rhs) {
        return  ((getMax(lhs.maxX, rhs.maxX) == rhs.maxX) && (getMax(lhs.maxY, rhs.maxY) == rhs.maxY) &&
                (getMin(lhs.minX, rhs.minX) == rhs.minX) && (getMax(lhs.minY, rhs.minY) == rhs.minY) );
    }
};
